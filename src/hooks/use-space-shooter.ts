...
import type { SpaceShooterScore } from "@/types";
...
function normalizeLeaderboardRows(rows: LeaderboardRow[] | null): SpaceShooterScore[] {
  return (rows || []).map((row) => ({
    ...row,
...
export function useSpaceShooterLeaderboard() {
  const [entries, setEntries] = useState<SpaceShooterScore[]>([]);
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    const leaderboardRes = await supabase
      .from("space_shooter_scores")
      .select(
        "id, player_id, best_score, best_duration_ms, created_at, updated_at, player:profiles!space_shooter_scores_player_id_fkey(id, full_name, avatar_url)"
      )
  ...
  const submitScore = useCallback(
    async (score: number, durationMs: number): Promise<SubmitScoreResult> => {
      ...
      if (existingRes.data) {
        ...
        const { error } = await supabase.from("space_shooter_scores").insert({
          player_id: user.id,
          best_score: roundedScore,
          best_duration_ms: roundedDuration,
        });
